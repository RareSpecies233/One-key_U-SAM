...
def visualize(idx, dice, image, mask, gt, pts):
    """
    Visualization function for evaluation results.
    Can be extended to save visualization images if needed.
    """
    # For now, just print the information
    # If visualization is needed, uncomment the following code:
    # import os
    ...
    # axes[1].set_title(f'Prediction (Dice: {dice:.4f})')
    # axes[1].axis('off')
    # axes[2].imshow(gt, cmap='gray')
    # axes[2].set_title('Ground Truth')
    # axes[2].axis('off')
    # plt.tight_layout()
    # plt.savefig(f'visualizations/sample_{idx}_dice_{dice:.4f}.png')
    # plt.close()
    pass
...
class BoundaryLoss(nn.Module):
    ...
    def _compute_distance_map(self, mask):
        ...
        for i in range(mask_np.shape[0]):
            # 计算到边界的距离（边界为0，内部为正距离）
            # 先计算到mask外部的距离，然后取负
            dist_out = distance_transform_edt(mask_np[i] == 0)
            dist_in = distance_transform_edt(mask_np[i] > 0)
            # 边界损失：使用到边界的距离（边界处为0，越远越大）
            dist_map[i] = dist_out - dist_in
        
        return torch.from_numpy(dist_map).float().to(mask.device)
    ...
    def forward(self, inputs, target, softmax=False):
        ...
        # 计算真实mask的距离变换图（到边界的距离）
        gt_dist_map = self._compute_distance_map(target)  # [B, H, W]
        
        # 边界损失：使用预测概率加权距离图
        # 对于每个类别，计算预测概率与距离图的加权和
        loss = 0.0
        for c in range(1, self.n_classes):  # 跳过背景类
            # 获取类别c的预测概率
            pred_prob = inputs[:, c]  # [B, H, W]
            
            # 边界损失：预测概率越高但距离边界越远，损失越大
            # 使用预测概率加权距离图
            boundary_loss = torch.sum(pred_prob * gt_dist_map) / (torch.sum(pred_prob) + 1e-8)
            loss += boundary_loss
        
        return loss / (self.n_classes - 1)  # 平均所有类别（除了背景）
...
def train_one_epoch(args: argparse.Namespace,
    ...
    # Add tqdm progress bar for training (only on main process)
    is_main = utils.is_main_process()
    if is_main:
        pbar = tqdm(data_loader, desc=header, unit='batch', ncols=120, leave=True)
    else:
        pbar = data_loader
    ...
    for samples, targets in pbar:
        ...
        outputs, losses, loss_dice, loss_ce, loss_boundary = model(samples, targets)

        loss_dict = {
            'loss': losses.clone().detach(),
            'loss_dice': loss_dice.clone().detach(),
            'loss_ce': loss_ce.clone().detach(),
            'loss_boundary': loss_boundary.clone().detach(),
        }
        ...
        if is_main:
            ...
            current_dice = loss_dice.item()
            current_ce = loss_ce.item()
            current_boundary = loss_boundary.item()
            current_lr = optimizer.param_groups[0]["lr"]
            pbar.set_postfix({
                'loss': f'{current_loss:.4f}',
                'dice': f'{current_dice:.4f}',
                'ce': f'{current_ce:.4f}',
                'boundary': f'{current_boundary:.4f}',
                'lr': f'{current_lr:.6f}'
            })
...
def parse_args():
    ...
    parser.add_argument('--use_2_5d', action='store_true',
                        help='use 2.5D input (three adjacent slices as three channels)')
    ...
    parser.add_argument('--dice_weight', default=0.6, type=float,
                        help='weight for dice loss (default: 0.6). CE loss weight = 1 - dice_weight')
    parser.add_argument('--boundary_weight', default=0.1, type=float,
                        help='weight for boundary loss (default: 0.1)')
...
class SAM(nn.Module):
    def __init__(self, args):
        ...
        self.boundary_loss = BoundaryLoss(args.sam_num_classes)
        ...
        self.boundary_weight = args.boundary_weight

        # lightweight input fusion stem (keeps 3-channel output)
    ...
    def forward(self, samples, targets=None):
        ...
        # pass normalized input through lightweight fusion stem
        samples.tensors = self.input_stem(samples.tensors)
        ...
        if self.training:
            ...
            def calc_loss(logits, labels, ce_loss, dice_loss, boundary_loss, dice_weight: float = 0.6, boundary_weight: float = 0.1):
                loss_ce = ce_loss(logits, labels)
                loss_dice = dice_loss(logits, labels, softmax=True)
                loss_boundary = boundary_loss(logits, labels, softmax=True)
                loss = (1 - dice_weight) * loss_ce + dice_weight * loss_dice + boundary_weight * loss_boundary
                return loss, loss_ce, loss_dice, loss_boundary

            gt = torch.stack([targets[i]['mask'] for i in range(len(targets))], dim=0)
            sam_losses, loss_ce, loss_dice, loss_boundary = calc_loss(masks, gt, self.ce_loss, self.dice_loss, self.boundary_loss, self.dice_weight, self.boundary_weight)
            return masks, sam_losses, loss_dice, loss_ce, loss_boundary
        else:
...
def main(args):
    ...
    if args.dataset == 'rectum':
        ...
        # RectumDataloader does not take `use_2_5d` in its __init__, so we only pass supported args
    ...
    prompt_suffix = '_2.5d' if args.use_2_5d else ''
    output_dir = Path(args.output_dir) / f'prompt={prompt}{prompt_suffix}'
    ...
    if args.resume:
        ...
        if 'optimizer' in checkpoint and 'epoch' in checkpoint:
            ...
            iter_num = checkpoint.get('iter_num', args.start_epoch * len(dataset_train))
            best_dice = checkpoint.get('best_dice', -1)
            early_stop_counter = checkpoint.get('early_stop_counter', 0)
            best_epoch = checkpoint.get('best_epoch', -1)
            print(f"Resumed from epoch {args.start_epoch}, best_dice: {best_dice:.6f}, "
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if mean_dice > best_dice:
            ...
            if args.output_dir:
                ...
                old_best_files = list(output_dir.glob('best_*.pth'))
                for old_file in old_best_files:
                    if old_file.exists():
                        old_file.unlink()
                checkpoint_paths.append(output_dir / f'best_{mean_dice:.6f}_{miou:.6f}.pth')
                print(f'New best model saved: dice={mean_dice:.6f}, miou={miou:.6f}')
        else:
            early_stop_counter += 1
            print(f'No improvement for {early_stop_counter} epoch(s). Best dice: {best_dice:.6f}')
        ...
        if args.output_dir:
            ...
            # Save regular checkpoint every epoch (for resume)
            checkpoint_path_regular = output_dir / f'checkpoint_epoch_{epoch}.pth'
            utils.save_on_master({
                'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
                'epoch': epoch,
            ...
            if epoch > args.start_epoch:
                prev_checkpoint = output_dir / f'checkpoint_epoch_{epoch-1}.pth'
        ...
        if early_stop_counter >= early_stop_patience:
            ...
            print(f'Best dice: {best_dice:.6f} at epoch {best_epoch}')
            break
