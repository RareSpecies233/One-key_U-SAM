...
class USAMExporter(nn.Module):
    """Wrapper for U-SAM model to facilitate ONNX export."""
    ...
    def forward(self, x):
        """
        Forward pass for inference (without prompts).
        Input: [B, 3, H, W] - batch of RGB images
        Output: [B, num_classes, H, W] - segmentation masks
        """
        device = x.device
